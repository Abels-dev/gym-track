...
interface SelectedExercise {
  id: string; // our temporary id for the list item
  exerciseId: string;
  name: string;
  targetSets: number;
  targetRepMin: number;
    ...
    <div className="flex flex-col flex-1 p-4 sm:p-6 md:p-8 max-w-3xl mx-auto w-full relative">
      <header className="flex items-center justify-between pb-6 mb-6 border-b border-border sticky top-0 bg-background z-10 pt-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-light tracking-tight">New Routine</h1>
        </div>
        <button
          onClick={handleSubmit}
          disabled={!name || exercises.length === 0 || createMutation.isPending}
