...
function WorkoutContent() {
  ...
  // 5. Add Exercise(s) to Active Workout Mutation
  const addExercisesMutation = useMutation({
    mutationFn: async (exerciseIds: string[]) => {
      if (!activeWorkout || exerciseIds.length === 0) return;
  ...
  return (
    <>
        ...
        <div className="flex flex-col flex-1 p-4 sm:p-6 md:p-8 lg:p-10 max-w-7xl mx-auto w-full pb-32 relative">
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                ...
                <h1 className="text-xl sm:text-2xl font-light tracking-tight truncate max-w-[200px] sm:max-w-md">
                  {activeWorkout.routine?.name || "Active Workout"}
                </h1>
              </div>
            </div>
          ...
          <div className="space-y-6 mb-8">
            {(activeWorkout.exercises || []).length === 0 ? (
              ...
              (activeWorkout.exercises || []).map((item: WorkoutExerciseItem) => (
                <WorkoutExerciseCard
                  key={item.id}
          ...
          {isPickerOpen && (
            <ExercisePicker
              ...
              onSelectMultiple={(chosenExercises) =>
                addExercisesMutation.mutate(chosenExercises.map((e) => e.id))
              }
              selectedIds={(activeWorkout.exercises || []).map((e: any) => e.exerciseId)}
            />
          )}
      ...
      {/* Workout Completion Ceremony Summary - Placed at root so it stays mounted */}
      {completedSummary && (
        <WorkoutSummaryModal
